...
class DeepSets(nn.Module):
    def __init__(
        self, input_dim, hidden_dim, output_dim, aggregator="sum", dropout=0.1
    ):
        super().__init__()
...
class GraphSetConv(nn.Module):
    def __init__(
        ...
        self.gcn_dropout = nn.Dropout(gcn_dropout if gcn_dropout is not None else ffn_dropout)
        self.act = self._build_activation(activation)
        ...
        self.ln_post = nn.LayerNorm(filters)
...
class GraphSetTransformerClassifier(nn.Module):
    def __init__(self, in_channels, hidden_dim, num_classes, dropout=0.2):
        super().__init__()
        self.setconv1 = GraphSetConv(
            filters=hidden_dim, in_channels=in_channels,
            activation="silu", mhsa_dropout=dropout, ffn_dropout=dropout,
        )
        self.setconv2 = GraphSetConv(
            filters=hidden_dim, in_channels=hidden_dim,
            activation="silu", mhsa_dropout=dropout, ffn_dropout=dropout,
        )
        ...
        # Pool nodes -> graph embedding (attention)
        ...
        # Pool graphs -> set embedding (attention)
    ...
    def forward(self, data, set_batch):
        ...
        graph_emb = self.graph_pool(x, batch)             # [num_graphs, H]
        set_emb = self.set_pool(graph_emb, set_batch)     # [num_sets, H]
        return self.classifier(set_emb)
