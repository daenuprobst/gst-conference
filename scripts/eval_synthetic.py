...
class GraphSetConvModel(nn.Module):
    """L stacked GraphSetConv blocks + node head. node_set_mode controls the
    cross-attn vs broadcast variant under test."""
...
def equalize_models(
    in_dim: int, hidden: int, num_layers: int, num_heads: int, task: str, mode: str
    ...
    base_params = {
        k: count_params(build_model(k, in_dim, hidden, num_layers, num_heads, task))
        for k in MODEL_KEYS
    }
    if mode == "none":
    ...
    def builder_for(key):
        return lambda h: build_model(key, in_dim, h, num_layers, num_heads, task)

    if mode == "smaller":
        # Pick the smaller of the two pipeline param counts as target.
    ...
    if mode == "larger":
        target_p = max(base_params["gsc-ca"], base_params["gsc-bc"])
        hiddens = {k: base_hiddens[k] for k in ["gsc-ca", "gsc-bc"]}
        params = {k: base_params[k] for k in ["gsc-ca", "gsc-bc"]}
...
def run(args):
    ...
    hiddens, params = equalize_models(
        in_dim, args.hidden, args.layers, args.num_heads, "ring", args.equalize_params
    )
