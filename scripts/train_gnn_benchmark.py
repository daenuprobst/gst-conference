"""Training script for GNNBenchmarkDataset (MNIST, CIFAR10)."""
import torch
...
def main():
    ...
    train_set_sizes = [5, 10, 20]  # Randomly vary during training
    eval_set_size = 1  # Fixed size for validation/test
    seeds = [10, 20, 30, 40, 50]
    
    print(f"\n{'#'*70}")
    print(f"{dataset_name} Grid Search Training")
    print(f"Official GNNBenchmarkDataset Splits")
    print(f"Train Set Sizes (random per epoch): {train_set_sizes}")
    print(f"Eval Set Size: {eval_set_size}")
    print(f"Seeds: {seeds}")
    ...
    base_output_dir = Path('../results') / dataset_name / f"{dataset_name}_set_varied"
    ...
    for seed in seeds:
        ...
        print(f"Train Set Sizes: {train_set_sizes}, Eval Set Size: {eval_set_size}, Seed: {seed}")
        print(f"{'='*70}\n")
        ...
        all_results = {
            'SetTransformer': {'train_loss': [], 'val_auroc': [], 'val_auprc': [], 'test_auroc': None, 'test_auprc': None, 'best_model': None},
            'DeepSets': {'train_loss': [], 'val_auroc': [], 'val_auprc': [], 'test_auroc': None, 'test_auprc': None, 'best_model': None},
            'GraphSetConv': {'train_loss': [], 'val_auroc': [], 'val_auprc': [], 'test_auroc': None, 'test_auprc': None, 'best_model': None}
        }
        
        # Create val/test sets with size=1 (individual graphs for evaluation)
        print(f"Creating evaluation sets (size={eval_set_size})...")
        val_sets = make_label_homogeneous_sets(list(val_dataset), eval_set_size)
        test_sets = make_label_homogeneous_sets(list(test_dataset), eval_set_size)
        print(f"Created sets: Val={len(val_sets)}, Test={len(test_sets)}")
        
        # Create val/test DataLoaders (fixed throughout training)
        val_loader = TorchDataLoader(SetDataset(val_sets), batch_size=batch_size, shuffle=False, collate_fn=collate_sets)
        test_loader = TorchDataLoader(SetDataset(test_sets), batch_size=batch_size, shuffle=False, collate_fn=collate_sets)
        ...
        for model_name in model_names:
            ...
            for epoch in range(num_epochs):
                # Recreate train sets each epoch with random size from train_set_sizes
                epoch_set_size = random.choice(train_set_sizes)
                train_sets = make_label_homogeneous_sets(list(train_dataset), epoch_set_size)
                train_loader = TorchDataLoader(SetDataset(train_sets), batch_size=batch_size, shuffle=True, collate_fn=collate_sets)
                ...
                if (epoch + 1) % 20 == 0:
                    print(f"  Epoch {epoch+1}/{num_epochs} (set_size={epoch_set_size}) - Loss: {train_loss:.4f}, Val AUROC: {val_auroc:.4f}, Val AUPRC: {val_auprc:.4f}")
        ...
        axes[1, 1].bar(model_names, test_aurocs, color=['#1f77b4', '#ff7f0e', '#2ca02c'])
        ...
        for i, (name, auroc) in enumerate(zip(model_names, test_aurocs)):
            axes[1, 1].text(i, auroc + 0.02, f'{auroc:.4f}', ha='center', va='bottom')
        
        axes[1, 2].bar(model_names, test_auprcs, color=['#1f77b4', '#ff7f0e', '#2ca02c'])
        ...
        for i, (name, auprc) in enumerate(zip(model_names, test_auprcs)):
            axes[1, 2].text(i, auprc + 0.02, f'{auprc:.4f}', ha='center', va='bottom')
        ...
        summary = pd.DataFrame({
            ...
            'Best Val AUPRC': [max(all_results[m]['val_auprc']) for m in model_names],
    ...
    print(f"\n{'='*70}")
    print(f"AGGREGATED RESULTS (Train sizes: {train_set_sizes}, Eval size: {eval_set_size})")
    print(f"{'='*70}\n")
    
    aggregated_summary = pd.DataFrame({
        'Model': model_names,
        'Val_AUROC_Mean': [np.mean(all_seed_results[m]['val_auroc']) for m in model_names],
        'Val_AUROC_Std': [np.std(all_seed_results[m]['val_auroc']) for m in model_names],
        'Val_AUPRC_Mean': [np.mean(all_seed_results[m]['val_auprc']) for m in model_names],
        'Val_AUPRC_Std': [np.std(all_seed_results[m]['val_auprc']) for m in model_names],
        'Test_AUROC_Mean': [np.mean(all_seed_results[m]['test_auroc']) for m in model_names],
        'Test_AUROC_Std': [np.std(all_seed_results[m]['test_auroc']) for m in model_names],
        'Test_AUPRC_Mean': [np.mean(all_seed_results[m]['test_auprc']) for m in model_names],
        'Test_AUPRC_Std': [np.std(all_seed_results[m]['test_auprc']) for m in model_names],
    })
