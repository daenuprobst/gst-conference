...
def main():
    ...
    print(f"Seeds: {seeds}")
    print(f"Total experiments: {len(set_sizes)} × {len(seeds)} = {len(set_sizes) * len(seeds)}")
    print(f"{'#'*70}\n")
    ...
    for set_size in set_sizes:
        ...
        base_output_dir = Path('../results') / dataset_name / f"{dataset_name}_set_{set_size}"
        ...
        for seed in seeds:
            ...
            print(f"Created sets: Train={len(train_sets)}, Val={len(val_sets)}, Test={len(test_sets)}")
