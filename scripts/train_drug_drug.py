...
def main():
    ...
    for seed_idx, seed in enumerate(seeds):
        ...
        all_results = {name: {'train_loss': [], 'val_acc': [], 'val_f1': [], 
                              ...
                              'test_auroc': None, 'test_auprc': None, 'best_model': None} 
        ...
        for model_name in model_names:
            ...
            for epoch in range(num_epochs):
                ...
                scheduler.step()
                val_acc, val_f1, val_auroc, val_auprc = evaluate(model, val_loader, device, num_classes)
                ...
                if (epoch + 1) % 10 == 0:
                    print(f"  Epoch {epoch+1}/{num_epochs} - Loss: {train_loss:.4f}, Val Acc: {val_acc:.4f}, Val F1: {val_f1:.4f}, AUROC: {val_auroc:.4f}, AUPRC: {val_auprc:.4f}")

            # Test evaluation with best model
            model.load_state_dict(all_results[model_name]['best_model'])
            test_acc, test_f1, test_auroc, test_auprc = evaluate(model, test_loader, device, num_classes)
            ...
            print(f"Best Val Acc: {best_val_acc:.4f}, Val F1: {best_val_f1:.4f}, AUROC: {best_val_auroc:.4f}, AUPRC: {best_val_auprc:.4f}")
            print(f"Test Acc: {test_acc:.4f}, Test F1: {test_f1:.4f}, AUROC: {test_auroc:.4f}, AUPRC: {test_auprc:.4f}")
